...
export async function getDetailedFetch(countryCode) {
	try {
		const detailedFetch = await fetch(
			`https://restcountries.com/v3/alpha/${countryCode}?fields=name;nativeName;population;region;subregion;capital;topLevelDomain;currencies;languages;borders;flag`
		);
